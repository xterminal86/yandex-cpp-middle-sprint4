...
FunctionExtractor::GetNameLocation(const std::string& function_ast)
{
  ...
  std::string coords = function_ast.substr(coord_start + 1,
  ...
  std::string end_coords = function_ast.substr(dash + 1,
                                               end_bracket - dash - 1);
                                      ...
                                      const FunctionNameLocation& func_loc)
{
  ...
  while ((class_pos = ast.find(class_marker, class_pos)) != std::string::npos)
  {
    ...
    std::string end_coords = ast.substr(
      second_coord_start + 1,
      second_coord_end - second_coord_start - 1
    );
...
std::string
FunctionExtractor::GetClassNameFromSource(const ClassInfo& class_info,
                                          const std::vector<std::string>& lines)
{
